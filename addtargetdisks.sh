...
disks=(`lsblk -nS -o name,serial,vendor | grep -v $bootdisk | grep -v sr0 |  grep -v LIO | awk '{print $1}'`)
nodes=(`docker exec etcdclient /TopStor/etcdgetlocal.py Active --prefix | awk -F'Partners/' '{print $2}' | awk -F"'" '{print $1}'`)
diskids=`lsblk -nS -o name,serial,vendor | grep -v $bootdisk | grep -v sr0 | grep -v LIO | awk '{print $1" "$2}'`
