...
for pos in possible:
 ...
 if posname in str(active):
  ...
  put('sync/known/Add_'+posname+'_'+pos[1]+'/request','known_'+'known_'+str(stamp()))
  put('sync/known/Add_'+posname+'_'+pos[1]+'/request/'+myhost,'known_'+'known_'+str(stamp()))
#  broadcasttolocal('known/'+posname,pos[1])
  ...
  print('############')
  put('sync/alias/Add_'+posname+'_'+str(aliast).replace('_',':::').replace('/',':::')+'/request','alias_'+str(stamp()))
  put('sync/alias/Add_'+posname+'_'+str(aliast).replace('_',':::').replace('/',':::')+'/request/'+myhost,'alias_'+str(stamp()))
...
if possible != []:
 for x in possible:
  ...
  put('sync/known/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request','known_'+str(stamp()))
  put('sync/known/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request/'+myhost,'known_'+str(stamp()))
  ...
  put('sync/ActivePartners/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request','ActivePartners_'+str(stamp()))
  put('sync/ActivePartners/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request/'+myhost,'ActivePartners_'+str(stamp()))

  put('hostipsubnet/'+x[0].replace('possible',''),hostsubnet)
  put('sync/hostipsubnet/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request/'+myhost,'hostipsubnet_'+str(stamp()))
  put('sync/hostipsubnet/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request','hostipsubnet_'+str(stamp()))
  put('configured/'+x[0].replace('possible',''),'yes')
  put('sync/configured/Add_'+x[0].replace('possible','')+'_yes/request','configured_'+str(stamp()))
  put('sync/configured/Add_'+x[0].replace('possible','')+'_yes/request/'+myhost,'configured_'+str(stamp()))
  put('nextlead',x[0].replace('possible','')+'/'+x[1])
  put('sync/nextlead/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request/'+myhost,'nextlead_'+str(stamp()))
  put('sync/nextlead/Add_'+x[0].replace('possible','')+'_'+x[1]+'/request','nextlead_'+str(stamp()))
  aliast = getlocal(x[1],'alias/'+x[0].replace('possible',''))[0]
  put('alias/'+x[0].replace('possible',''),str(aliast))
  put('sync/alias/Add_'+x[0].replace('possible','')+'_'+x[1].replace('_',':::').replace('/',':::')+'/request/'+myhost,'alias_'+str(stamp()))
  put('sync/alias/Add'+x[0].replace('possible','')+'_'+x[1].replace('_',':::').replace('/',':::')+'/request','alias_'+str(stamp()))
...
#  broadcast('broadcast','/TopStor/pump.sh','syncnext.sh','nextlead','nextlead')
  if x[0].replace('possible','') in str(knowns):
   ...
   put('sync/allowedPartners/Add_notoall_/request','allwedPartners_'+str(stamp()))
   put('sync/allowedPartners/Add_notoall_/request/'+myhost,'allwedPartners_'+str(stamp()))
