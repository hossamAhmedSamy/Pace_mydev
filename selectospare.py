...
#####################################  set the right replacements for all raids
 ...
 for raid in allraids:
    if raid['pool']  in str(exception):
     ...
     continue
    '''
    _, raid_min_dsize= min((norm(disk['size']),disk['size']) for disk in raid['disklist'])
    print(raid_min_dsize)
    disks_best_located = set()
    disks_need_relocation = set()
    raid_hosts = set()
    for disk in raid['disklist']:
        if disk['size'] != raid_min_dsize or disk['host'] in raid_hosts:
            disks_need_relocation.add(disk['name'])
        else:
            raid_hosts.add(disk['host'])
    print(raid_hosts)
    print(disks_need_relocation)
    '''         
